...
class Zoom_BBox(Operation):
    ...
    """
    def __init__(self, probability, min_factor, max_factor, txt_directory):
        """
    ...
    def perform_operation(self, images):
        ...
        txt_directory = self.txt_directory
        ...
        def do_label(image, txt_directory):
            w, h = image.size
            
            label_txt = open(txt_directory, "r")
            lines = label_txt.read().split(' ')
            
            bbox_sample = np.zeros((h,w,3), dtype=np.uint8)
            
            class_label = int(lines[0])
            x = float(lines[1])
            y = float(lines[2])
            x = int(x*w)
            y = int(y*h)
            ...
            o = np.array(([255]),dtype=np.uint8)
            
            bbox_sample[int(y):int(y)+5,int(x):int(x)+5] = o
            bbox_sample[int(y)-5:int(y),int(x)-5:int(x)] = o 
            
            augmented_bbox = Image.fromarray(bbox_sample)
            
            

            augmented_zoomed = augmented_bbox.resize((int(round(image.size[0] * factor)),
                                         int(round(image.size[1] * factor))),
                                         resample=Image.BICUBIC)
            w_zoomed, h_zoomed = augmented_zoomed.size
            
            f_w = float(int(b_w*factor)/w)
            f_h = float(int(b_h*factor)/h)
            

            augmented_zoomed.crop((floor((float(w_zoomed) / 2) - (float(w) / 2)),
                                      floor((float(h_zoomed) / 2) - (float(h) / 2)),
                                      floor((float(w_zoomed) / 2) + (float(w) / 2)),
                                      floor((float(h_zoomed) / 2) + (float(h) / 2))))
            return augmented_zoomed, f_w, f_h , class_label
        ...
        for image in images:
            ...
            augmented_zoomed, f_w, f_h, class_label = do_label(image,txt_directory)
            augmented_bboxes.append(augmented_zoomed)
            f_w_set.append(f_w)
            f_h_set.append(f_h)
            label_set.append(class_label)
        
        count = 0
        for augmented_bbox in augmented_bboxes:
            augmented_bbox = np.array(augmented_bbox)

            h = int(augmented_bbox[:,0,0].size)
            w = int(augmented_bbox[0,:,0].size)
            
            center = np.where(augmented_bbox[:,:,0]>254)
            n = center[0].size            
            cy = np.split(center[0],n)
            cx = np.split(center[1],n)
            #print('center',center)
            ...
            class_label = int(label_set[count])
            f_w = float(f_w_set[count])
            f_h = float(f_h_set[count])
            count = count+1
            ...
            co = class_label, bbox_x,bbox_y,bbox_w,bbox_h

        return augmented_images, co

class Flip_BBox(Operation):
    """
    ...
    """
    def __init__(self, probability, top_bottom_left_right, txt_directory):
        """
    ...
    def perform_operation(self, images):
        ...
        def do_label(image, txt_directory):
            ...
            bbox_sample[int(y):int(y)+2,int(x):int(x)+2] = o
            bbox_sample[int(y)-2:int(y),int(x)-2:int(x)] = o            
        
            augmented_bbox = Image.fromarray(bbox_sample)
            
            if self.top_bottom_left_right == "LEFT_RIGHT":
                augmented_bbox.transpose(Image.FLIP_LEFT_RIGHT)
            elif self.top_bottom_left_right == "TOP_BOTTOM":
                augmented_bbox.transpose(Image.FLIP_TOP_BOTTOM)
            elif self.top_bottom_left_right == "RANDOM":
                if random_axis == 0:
                    augmented_bbox.transpose(Image.FLIP_LEFT_RIGHT)
                elif random_axis == 1:
                    augmented_bbox.transpose(Image.FLIP_TOP_BOTTOM)
            return augmented_bbox, f_w, f_h , class_label
        ...
        for image in images:
            augmented_images.append(do(image))
            augmented_bbox, f_w, f_h, class_label = do_label(image,txt_directory)
...
class FlipRotateGaussian_BBox(Operation):

    def __init__(self, probability, max_left_rotation, max_right_rotation, ksize, txt_directory, sigmaX_min, sigmaX_max, expand=True):
    ...
    def perform_operation(self, images):
        ...
        def do_label(image, txt_directory):
            ...
            bbox_point = np.zeros((h,w,3), dtype=np.uint8)
            bbox_point[int(y):int(y)+2,int(x):int(x)+2] = o
            bbox_point[int(y)-2:int(y),int(x)-2:int(x)] = o
            
            new_bbox = np.zeros((h*3,w*3,3), dtype=np.uint8)
            
            new_bbox[:h, :w, :] = bbox_sample
            new_bbox[:h, w:2*w, :] = bbox_sample
            new_bbox[:h, 2*w:, :] = bbox_sample
            
            new_bbox[h:2*h, :w, :] = bbox_sample
            new_bbox[h:2*h, w:2*w, :] = bbox_point
            new_bbox[h:2*h, 2*w:, :] = bbox_point
            
            new_bbox[2*h:, :w, :] = bbox_sample
            new_bbox[2*h:, w:2*w, :] = bbox_sample
            new_bbox[2*h:, 2*w:, :] = bbox_sample
                      
            bbox = Image.fromarray(new_bbox)
            bbox = bbox.rotate(rotation, expand=self.expand, resample=Image.BICUBIC)            
            bbox_sample = Image.fromarray(bbox_sample)
            
            a,b = bbox_sample.size
            W, H = bbox.size
            
            augmented_bbox = bbox.crop(((W/2)-(a/2), (H/2)-(b/2), (W/2)+(a/2), (H/2)+(b/2)))
            return augmented_bbox, f_w, f_h, class_label
        ...
        for augmented_bbox in augmented_bboxes:
            ...
            #print('0 %f %f %f %f' %(bbox_x,bbox_y,bbox_w,bbox_h))            

            co = class_label, bbox_x,bbox_y,bbox_w,bbox_h
            #print(center)
            #if count == 0:
           #     file.write('0 %f %f %f %f' %(bbox_x,bbox_y,bbox_w,bbox_h))                    
          #  else:
         #       file.write('\n\n0 %f %f %f %f' %(bbox_x,bbox_y,bbox_w,bbox_h))
        #file.close()
...
class FlipRotate(Operation):
    ...
    def __init__(self, probability, max_left_rotation, max_right_rotation, expand=True):
...
class GaussianBlur(Operation):
    ...
    def perform_operation(self, images):
        ...
        def do(image):            
            ...
            sigmaX = random.uniform(self.sigamX_min,self.sigmaX_max)
        ...
        for image in images:
            augmented_images.append(Image.fromarray(do(np.array(image))))
