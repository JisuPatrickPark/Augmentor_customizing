...
class Pipeline(object):
    ...
    def _execute(self, augmentor_image, save_to_disk=True, multi_threaded=True):
        ...
        for operation in self.operations:
            ...
            if r <= operation.probability:
                images, coordinates = operation.perform_operation(images)
            ...
            try:
                ...
                for i in range(len(images)):
                    if i == 0:
                        ...
                        txt_name = augmentor_image.class_label + "_original_" + os.path.basename(augmentor_image.image_path) + "_" + file_name
                        np.savetxt(txt_name+'.txt', coordinates[None], fmt = '%f')
                    else:
                        save_name = "_groundtruth_(" + str(i) + ")_" + augmentor_image.class_label + "_" + os.path.basename(augmentor_image.image_path) + "_" + file_name \
                                    + "." + (self.save_format if self.save_format else augmentor_image.file_format)
                        images[i].save(os.path.join(augmentor_image.output_directory, save_name))
                        txt_name =  "_groundtruth_(" + str(i) + ")_" + augmentor_image.class_label + "_" + os.path.basename(augmentor_image.image_path) + "_" + file_name
                        
                        np.savetxt(txt_name+'.txt', coordinates[None], fmt = '%d', delimiter=' ')
    ...
    def gaussian_blur(self, probability, ksize, sigmaX_min=0, sigmaX_max=0):
        ...
        if not 0 < probability <= 1:
            ...
            self.add_operation(GaussianBlur(probability=probability, ksize = ksize, sigmaX_min = sigmaX_min, sigmaX_max = sigmaX_max))
    ...
    def frg_bbox(self, probability, max_left_rotation, max_right_rotation, ksize, txt_directory, sigmaX_min=0, sigmaX_max=0, expand=True):
        ...
        if not 0 <= max_right_rotation <= 359:
            ...
            self.add_operation(FlipRotateGaussian_BBox(probability=probability, max_left_rotation=ceil(max_left_rotation), max_right_rotation=ceil(max_right_rotation), ksize = ksize, sigmaX_min = sigmaX_min, sigmaX_max = sigmaX_max, txt_directory=txt_directory, expand=expand))
            
    def flip_random_bbox(self, probability, txt_directory):

        if not 0 < probability <= 1:
            raise ValueError(Pipeline._probability_error_text)
        else:
            self.add_operation(Flip_BBox(probability=probability, top_bottom_left_right="RANDOM", txt_directory=txt_directory))
            
    def zoom_bbox(self, probability, min_factor, max_factor, txt_directory):
        ...
        if not 0 < probability <= 1:
            ...
            self.add_operation(Zoom_BBox(probability=probability, min_factor=min_factor, max_factor=max_factor, txt_directory = txt_directory))
